												   ...
												   VkImage existingImage)
{
	...
	if (existingImage == nullptr)
	{
		...
		if (hostVisible)
		{
			...
			GP_ASSERT(flags != 0, "Format is not supported for host visible images");
		}
		else
		{
			VkFormatFeatureFlags flags = formatProps.optimalTilingFeatures & formatFeatureFlags;
			GP_ASSERT(flags != 0, "Format is not supported for GPU local images (i.e. not host visible images)");
		}
