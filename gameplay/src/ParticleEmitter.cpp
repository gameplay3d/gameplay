...
ParticleEmitter::ParticleEmitter(SpriteBatch* batch, unsigned int particleCountMax) :
    _spriteBatch(batch), _particleCountMax(particleCountMax), _particleCount(0), _particles(NULL),
    _emissionRate(EMISSION_RATE), _active(false), _ellipsoid(false),
    ...
    _spriteTextureBlending(BLEND_TRANSPARENT),  _spriteTextureWidth(0), _spriteTextureHeight(0), _spriteTextureWidthRatio(0), _spriteTextureHeightRatio(0), _spriteTextureCoords(NULL), 
...
void ParticleEmitter::setTextureBlending(TextureBlending textureBlending)
{
    ...
    {
        case BLEND_OPAQUE:
            _spriteBatch->setBlendEnabled(false);
            break;
        case BLEND_TRANSPARENT:
            _spriteBatch->setBlendEnabled(true);
            _spriteBatch->setBlendMode(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
            break;
        case BLEND_ADDITIVE:
            _spriteBatch->setBlendEnabled(true);
            _spriteBatch->setBlendMode(GL_ONE, GL_ONE);
            break;
        case BLEND_MULTIPLIED:
            _spriteBatch->setBlendEnabled(true);
            _spriteBatch->setBlendMode(GL_ZERO, GL_SRC_COLOR);
            break;
    }
}
...
inline void ParticleEmitter::generateColor(const Vector4& base, const Vector4& variance, Vector4* dst)
{
