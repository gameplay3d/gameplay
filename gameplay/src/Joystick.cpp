...
Joystick::Joystick() : _relative(true)
{
}
...
bool Joystick::touchEvent(Touch::TouchEvent touchEvent, int x, int y, unsigned int contactIndex)
{
    switch (touchEvent)
    {
        case Touch::TOUCH_PRESS:
        {
            ...
            {
                _screenRegion.x = x + _bounds.x - _outerSize->x * 0.5f;
                _screenRegion.y = y + _bounds.y - _outerSize->y * 0.5f;
            }
        ...
        {
            float dx = x - ((_relative) ? _screenRegion.x - _bounds.x : 0.0f) - _outerSize->x * 0.5f;
            float dy = -(y - ((_relative) ? _screenRegion.y - _bounds.y : 0.0f) - _outerSize->y * 0.5f);
