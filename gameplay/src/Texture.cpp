...
Texture* Texture::create(TextureHandle handle, int width, int height, Format format)
{
	...
	{
		texture->_type = TEX_2D; //TODO figure out if 2D or cube
	}
    texture->_width = width;
    texture->_height = height;
...
Texture* Texture::createCube(Format format, unsigned int width, unsigned int height, unsigned char* data[], bool generateMipmaps)
{
	// Create the texture.
    GLuint textureId;
    GL_ASSERT( glGenTextures(1, &textureId) );
