...
void PostProcessTest::initialize()
{
	...
	_frameBuffer = FrameBuffer::create("fb", getWidth(), getHeight());
	DepthStencilTarget* dst = DepthStencilTarget::create("", DepthStencilTarget::DEPTH_STENCIL, getWidth(), getHeight());
	...
	_compositors.push_back(compositor);
	compositor->getMaterial()->getParameter("u_width")->setValue((float)getWidth());
	compositor->getMaterial()->getParameter("u_height")->setValue((float)getHeight());

	compositor = Compositor::create(_frameBuffer, NULL, "res/common/postprocess.material", "Gaussian Blur");
	_compositors.push_back(compositor);
	compositor->getMaterial()->getParameter("u_length")->setValue(1.0f / 256.0f);
...
void PostProcessTest::render(float elapsedTime)
{
	...
	Rectangle prevViewport = Game::getInstance()->getViewport();
	...
	Compositor* compositor = _compositors[_compositorIndex];
    compositor->blit(prevViewport, true);
	...
	Game::getInstance()->setViewport(prevViewport);
}
