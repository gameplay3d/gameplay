...
#if defined(ADD_TEST)
    ADD_TEST("Input", "Gestures", GestureTest, 2);
#endif
#endif
...
void GestureTest::update(float elapsedTime)
{
}

void GestureTest::render(float elapsedTime)
{
...
void GestureTest::touchEvent(Touch::TouchEvent evt, int x, int y, unsigned int contactIndex)
{
    switch (evt)
    {
...
void GestureTest::gestureSwipeEvent(int x, int y, int direction)
{
...
void GestureTest::gestureTapEvent(int x, int y)
{
    std::ostringstream convert;
    convert << "Tap " << x << ", " << y;
    _eventLog.push_front(convert.str());
