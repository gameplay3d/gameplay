#include "CreateSceneTest.h"
#include "TestsGame.h"

#if defined(ADD_TEST)
    ADD_TEST("Scene", "Create Scene", CreateSceneTest, 1);
#endif
...
void CreateSceneTest::render(float elapsedTime)
{
    ...
    _scene->visit(this, &CreateSceneTest::drawScene);

    drawFrameRate(_font, Vector4(0, 0.5f, 1, 1), 5, 1, getFrameRate());
}

void CreateSceneTest::touchEvent(Touch::TouchEvent evt, int x, int y, unsigned int contactIndex)
{
    switch (evt)
    {
...
bool CreateSceneTest::drawScene(Node* node)
{
